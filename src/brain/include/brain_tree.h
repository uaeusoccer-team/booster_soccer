...
public:
    ...
    static PortsList providedPorts()
    {
        return {
            ...
            InputPort<bool>("adaptive_kick_range", true, "Adjust kick range based on ball alignment quality"),
            InputPort<double>("well_aligned_kick_range", 0.85, "Kick range when the ball is well centered"),
            InputPort<double>("poor_aligned_kick_range", 0.55, "Kick range when the ball alignment is poor"),
            InputPort<double>("well_aligned_yaw_abs", 0.18, "Yaw threshold for well-centered adaptive kick range"),
            InputPort<double>("well_aligned_lateral_abs", 0.10, "Lateral threshold for well-centered adaptive kick range"),
            InputPort<double>("poor_aligned_yaw_abs", 0.35, "Yaw threshold for close-only adaptive kick range"),
            InputPort<double>("poor_aligned_lateral_abs", 0.22, "Lateral threshold for close-only adaptive kick range"),
