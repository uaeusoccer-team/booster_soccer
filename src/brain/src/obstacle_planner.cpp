    ...
    double *clearanceOut = nullptr) noexcept
{
    ...
    // The instantaneous nearest-obstacle check protects the current pose.
    // Apply the same public stop envelope to the complete predicted path so a
    // scored detour cannot enter it and rely on a later emergency tick.
    const double requiredPathClearance = std::max(
        0.0,
        clampNonnegative(input.stopDistance) - corridorHalfWidth);
    return clearance > kEpsilon &&
        clearance + kEpsilon >= requiredPathClearance;
}
...
ObstaclePlannerOutput ObstacleVelocityPlanner::plan(const ObstaclePlannerInput &input)
{
    ...
    if (!input.enabled && equalsIgnoreCase(input.mode, "score")) {
        // Score is deliberately a guarded full-speed operation. Never let a
        // disabled/misconfigured filter turn it into an unchecked passthrough,
        // even if a command bypasses Autonomous_run.sh validation.
    ...
    if (equalsIgnoreCase(input.mode, "score")) {
        // Adjustment already chose the shooting line. At full scoring speed a
        // last-second lateral detour is unstable and can steer into a post, so
        // score is intentionally direct-or-stop.
        if (directClear) {
