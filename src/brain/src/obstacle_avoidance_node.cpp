...
BT::NodeStatus ObstacleAvoidance::tick()
{
    ...
    const bool scoreMode = mode == "score";
    bool occlusionPursuit =
        enabled && !scoreMode && expected_ball_valid_ && !rgbBallVisible;
    if (enabled && detectionStale && !scoreMode) {
    ...
    } else if (enabled && rgbBallVisible && !depthBallVisible && !scoreMode) {
    ...
    const bool inspectVisibleGap = enabled && !scoreMode && freshDepth && depthBallVisible &&
        translationalIntent &&
