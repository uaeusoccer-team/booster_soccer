...
// 
    ...
    // double circleSafeDist = isStriker ? max(safeDist, 0.8) : safeDist;
// 
// // 
//     Calculate target point
//     core chase-vector logic: 
//     decides whether to go directly behind the ball or circle around it.
    // if (fabs(toPInPI(kickDir - theta_rb)) < dirThreshold) {
        // log("targetType = direct");
        // targetType = "direct";
        // target_f.x = ballPos.x - dist * cos(kickDir);
    ...
    // } else {
        ...
        // circleBackDir = toPInPI(theta_br - kickDir) > cbDirThreshold ? 1.0 : -1.0;
        // log(format("targetType = circle_back, circleBackDir = %.1f", circleBackDir));
        // double tanTheta = theta_br + circleBackDir * acos(min(1.0, circleSafeDist/max(ballRange, 1e-5))); 
        // target_f.x = ballPos.x + circleSafeDist * cos(tanTheta);
        // target_f.y = ballPos.y + circleSafeDist * sin(tanTheta);
    // }
    // target_r = brain->data->field2robot(target_f);
            // 
    // double targetDir = atan2(target_r.y, target_r.x);
    ...
    // if (avoidObstacle && distToObstacle < oaSafeDist) {
        ...
        // vtheta = ballYaw;
    // } else {
        // if (isStriker && targetType == "circle_back") {
            // vx = cap(target_r.x * 0.6, vxLimit * 0.6, -vxLimit * 0.3);
            // vy = cap(target_r.y * 0.8, vyLimit, -vyLimit);
            // vtheta = ballYaw * 1.5;
        // } else {
            // vx = min(vxLimit, targetRange);
            // vy = 0;
            // vtheta = targetDir;
            // if (fabs(targetDir) < 0.1 && ballRange > 2.0) vtheta = 0.0;
            // vx *= sigmoid((fabs(vtheta)), 1, 3); 
