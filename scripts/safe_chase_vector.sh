...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/safe_chase_vector.sh [vx_limit=0.18] [vy_limit=0.06] [stop_dist=0.8] [stop_angle=0.2] [y_tolerance=0.03] [body_turn_speed=0.25] [head_turn_start_ratio=0.75] [head_turn_stop_ratio=0.65] [use_ball_yaw_fallback=true]

Examples:
  ./scripts/safe_chase_vector.sh vx_limit=0.20 vy_limit=0.06 stop_dist=1.8 stop_angle=0.1 y_tolerance=0.05 body_turn_speed=0.25 head_turn_start_ratio=0.75 head_turn_stop_ratio=0.65 use_ball_yaw_fallback=true

  ./scripts/safe_chase_vector.sh --vx-limit 0.20 --vy-limit 0.06 --stop-dist 1.8 --stop-angle 0.1 --y-tolerance 0.05 --body-turn-speed 0.25 --head-turn-start-ratio 0.75 --head-turn-stop-ratio 0.65 --use-ball-yaw-fallback true
...
set_simple_chase_value() {
  ...
  if [[ "$key" == "use_ball_yaw_fallback" ]]; then
    value="${value,,}"
    case "$value" in
      true|1) value="true" ;;
      false|0) value="false" ;;
  ...
  if [[ "$key" != "use_ball_yaw_fallback" ]] && ! [[ "$value" =~ $value_pattern ]]; then
  ...
  case "$key" in
    ...
    *)
      echo "Unknown safe chase setting: ${key}" >&2
      exit 2
      ;;
  esac
}
...
parse_block_text() {
  ...
  if [[ "$text" =~ (^|[[:space:]\{])use_ball_yaw_fallback[[:space:]]*=[[:space:]]*\"?(true|false|1|0)\"? ]]; then
    set_simple_chase_value use_ball_yaw_fallback "${BASH_REMATCH[2]}"
    matched=0
  fi
  ...
  while (($#)); do
    case "$1" in
      ...
      --head-turn-stop-ratio|--head_turn_stop_ratio)
        ...
        shift 2
        ;;
      --use-ball-yaw-fallback|--use_ball_yaw_fallback)
        [[ $# -ge 2 ]] || { echo "Missing value for $1" >&2; exit 2; }
        set_simple_chase_value use_ball_yaw_fallback "$2"
        shift 2
        ;;
      --vx-limit=*|--vx_limit=*)
      ...
      --head-turn-stop-ratio=*|--head_turn_stop_ratio=*)
        ...
        ;;
      --use-ball-yaw-fallback=*|--use_ball_yaw_fallback=*)
        set_simple_chase_value use_ball_yaw_fallback "${1#*=}"
        shift
        ;;
      vx_limit=*|vy_limit=*|stop_dist=*|stop_angle=*|y_tolerance=*|body_turn_speed=*|head_turn_start_ratio=*|head_turn_stop_ratio=*|use_ball_yaw_fallback=*)
    ...
    <Sequence name="root">
      ...
      <ReactiveSequence _while="gc_game_state!='END'" name="safe chase behind ball">
        ...
        <IfThenElse>
          ...
          <Sequence name="Find missing ball">
            <CamFindBall theta="{tracking_theta}" />
